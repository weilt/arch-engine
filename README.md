...
| **MCP Server** | 契约 / 架构 / 设计 / 本体共 **20** 个工具，代理必须调用 |
...
| 架构 | `query_path_rules` | 只读：path-rules.json 快照、arch.config java 段、pathRulesHash |
| 架构 | `update_java_path_rules` | 声明 Controller 路径前缀规则并可选触发 API reindex |
...
| 本体 | `query_ontology` | 项目全景快照（模块 / 资产 / 契约 / 设计 / 拓扑）或按 topic 深入 |
| 本体 | `query_impact` | 变更影响分析：实体 → 调用链、方法 → callers/callees、组件 → importers |
| 路径规则 | `update_java_path_rules` | 手动配置或修正 Java API URL 前缀规则（WebMvcRegistrations 解析的补充） |
| 路径规则 | `query_path_rules` | 查询当前生效的 Java API 路径规则与前缀映射 |
...
### 本体图谱与调用图（Ontology + Call Graph）

- **`query_ontology`**（v2.0.3+）：项目全景快照 — 模块 / 资产 / 契约 / 设计 / 拓扑（moduleCount、rpcEndpoints、entityCount、flowEdgeCount、crossServiceRefs、methodCount、dtoCount、callEdgeCount、importEdgeCount）。支持 `topic` 模式按模块/主题深入
- **`query_impact`**（v2.0.3+）：变更影响分析 — 输入实体名返回影响的层（repository/service/controller/frontend）；输入 `Class.method` 返回 callers/callees/运行时注解（@Transactional 等）；输入 DTO 名返回字段 + 引用方法；输入前端组件名返回 importers/imports/template 使用者
- **`start-init`** 产出三层图谱：`entities.json`（JPA/MyBatis/SQL 实体 + 关系）、`flow.json`（实体 → 后端链 → 前端链的类级流向）、`call-graph.json`（方法级调用 + DTO 字段 + 运行时注解 + 前端 import 图）
- **运行时注解**（v2.0.5）：扫描 6 类行为注解（@Transactional、@Cacheable/@CacheEvict、@PreAuthorize/@Secured、@Scheduled、@EventListener、@Async），附在方法节点上
- **前端 import 图**（v2.0.5）：ES import 边 + Vue 模板标签使用边，支持组件级影响分析
...
# 3. 新开 IDE / Codex 会话，确认 agent-protocol-mcp 已连接（20 个 APT 工具）
```
...
4. 确认列表中 **20 个 APT 工具**已连接；使用 `/feature` 或 `$apt-feature`。
...
- v2.0.3+：产出实体图谱 `entities.json`（JPA/MyBatis/SQL 实体 + 关系）、流向图谱 `flow.json`（实体 → 后端 → 前端的类级调用链）
- v2.0.5：产出调用图谱 `call-graph.json`（方法级调用边、DTO 字段、运行时注解、前端 import/template 边）
...
| `query_path_rules` | Java API 路径前缀诊断 | 只读返回 path-rules.json、arch.config java 段、pathRulesHash |
| `update_java_path_rules` | 纠正 Controller URL 前缀 | upsert 规则到 arch.config 并默认触发 API reindex |
...
| `query_ontology` | 需要了解项目全景 / 拓扑 | 返回模块、资产、契约、设计、拓扑指标（含方法/DTO 计数） |
| `query_impact` | 分析变更爆炸半径 | 实体 → 影响层；方法 → callers/callees/注解；DTO → 字段/引用；组件 → importers |
| `update_java_path_rules` | 手动修正 API 前缀规则 | 配置 `pathRules` 覆盖自动检测的 WebMvcRegistrations 前缀 |
| `query_path_rules` | 查看当前路径规则 | 返回自动检测 + 手动配置的 Java API 前缀规则全貌 |
...
**独立 starter 自动发现（无需 `WebMvcRegistrations` 入口）：** 当框架模块与业务仓分离、或项目采用非芋道默认写法时，`start-init` 还会按以下探测器补充规则（与上表合并，同 `controllerPattern` 不重复）：

- **WebProperties 直连**：任意含 `@ConfigurationProperties` 且 `new Api(prefix, pattern)` 字段默认值的 Java 类
- **WebMvcConfigurer**：`implements WebMvcConfigurer` 且含 `addPathPrefix` / `pathPrefixes.put` / `configurePathMatch`
- **AutoConfiguration 链**：读取 `META-INF/spring/org.springframework.boot.autoconfigure.AutoConfiguration.imports`（及 `spring.factories`）定位 `@AutoConfiguration` 类，再对其应用上述两种探测器
- **extraSourceRoots**：在 `arch.config.json` 的 `java.extraSourceRoots` 声明兄弟路径（如 `../company-framework`），仅用于 path-rules 发现，不自动加入 Maven 模块列表
- **yml-only 补充**：`application*.yml` 存在 `*.web.*.prefix` 与对应 `*.controller`，但工作区无 Java `Api` 默认值时，以 `confidence: medium`、`source: yml-only:…` 生成规则
...
4. 新开 ZCode 任务，确认 20 个工具可用。仅有命令无 MCP 时，代理无法执行契约/架构硬约束。
...
- [x] **本体图谱**（v2.0.3）：`query_ontology` + `query_impact`，实体/流向图谱，项目全景快照
- [x] **AST 实体扫描 + RPC 流向**（v2.0.4）：java-parser CST 实体扫描、Feign RPC 跨服务边、Scanner Registry 架构、ontology topology + topic drill-down
- [x] **方法级调用图 + 前端 impact + refresh_asset 修复**（v2.0.5）：call-graph.json（方法→方法调用、DTO 字段、6 类运行时注解）、前端 import/template 图、`query_impact` 升级（callers/callees/annotations/dto/components）、`refresh_asset` TS 源码分类修复

**计划中：**

- [ ] TS AST 级契约校验（当前仅校验文件存在）
- [ ] 前端完整数据流（路由 → store → api 链）（v2.0.6）
- [ ] 跨仓库 workspace + 多语言后端（C#/Go/Rust）（v2.1）
- [ ] tree-sitter WASM 全量 + 运行时数据源接入（v2.2）
...
- [x] **方法级调用图 + 前端 impact + refresh_asset 修复**（v2.0.5）：call-graph.json（方法→方法调用、DTO 字段、6 类运行时注解）、前端 import/template 图、`query_impact` 升级（callers/callees/annotations/dto/components）、`refresh_asset` TS 源码分类修复
- [x] **Java API 路径规则增强 + v0 页面交接**（v2.0.6）：yml-only / WebMvcConfigurer / AutoConfiguration 三级路径检测器、`update_java_path_rules` + `query_path_rules` MCP 工具、`--reindex-apis` CLI（API-only 刷新）、v0 页面交接（`apt-v0-handoff` skill + Phase A 冻结门禁）、page-rollout-spec 模板
