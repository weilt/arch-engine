...
| **MCP Server** | 契约 / 架构 / 设计 / 本体共 **18** 个工具，代理必须调用 |
...
# 3. 新开 IDE / Codex 会话，确认 agent-protocol-mcp 已连接（18 个 APT 工具）
```
...
4. 确认列表中 **18 个 APT 工具**已连接；使用 `/feature` 或 `$apt-feature`。
...
4. 新开 ZCode 任务，确认 18 个工具可用。仅有命令无 MCP 时，代理无法执行契约/架构硬约束。
