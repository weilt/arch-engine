...
Body:
- spec → /plan-from-spec → /implement-plan → /verify（含 UI 时设计 audit 只读）
- 口头描述 → /feature → /verify
- FAIL → /finish-feature 补救

### Visual
Two parallel pipeline schematics with decision fork at top. Both pipelines end with /verify box. Blueprint arrows between stages. Dashed arrow from /verify to /finish-feature labeled FAIL only.
