...
APT（Agent-Protocol-Toolkit）是一套**全局安装、按项目激活**的开发工具集，面向 Claude Code、Cursor、Qoder、Codex、**ZCode** 与 MCP。
...
| ZCode | .zcode/commands/ + .zcode/skills/apt-*/（$apt-*） |
| Codex | .agents/skills/apt-*/ |

全局安装一次，项目根 agent-init 激活。MCP 配置含 APT_PROJECT_ROOT。ZCode 的斜杠命令/Skill 自动生效；**MCP 需在 ZCode Settings → MCP Servers 中 Import 或手动添加** `agent-protocol-mcp`（项目 `.zcode/mcp.json` 作参考，未必被自动加载）。
