...
让 APT 从"单仓库 Java + 前端"升级到"多仓库、多语言"架构。企业微服务通常是几十个独立 repo（Java + Go + Python + 前端混部），APT 必须能跨 repo 扫描、建立统一知识图谱、回答跨服务影响链问题。本版本新增 Go 与 Python 语言 scanner，使 APT 覆盖 Java + Go + Python 三大主流后端组合（Go 覆盖云原生微服务，Python 覆盖 AI / 数据 / 脚本级微服务）。

一句话价值：让 AI agent 能在一个包含 Java 微服务、Go 微服务、Python 微服务、Vue 前端的多 repo 项目里做精确的跨服务影响分析。
...
- Python 语言 scanner（pyproject.toml/setup.py 模块发现 + HTTP API（FastAPI/Flask/Django/Tornado）+ gRPC proto + ORM/Pydantic/Django 实体 + 函数级调用图）
...
- 不做 C# / Rust scanner（v2.1.1+ 以插件增量加入）
...
2. `start-init --workspace-init` 自动扫描子目录生成 `apt-workspace.json`（检测 pom.xml / go.mod / pyproject.toml / setup.py / package.json / *.csproj）
...
5. Python repo 扫描产出：HTTP API（FastAPI/Flask/Django/Tornado）、gRPC 服务（.proto）、ORM/Pydantic/Django 实体、函数节点 + 调用边
6. `.ai/arch/backend/<repo-slug>/<module>/` 路径结构，arch-index 树含 repo 层
7. `query_ontology()` topology 含 repoCount + 每 repo 的模块/实体/方法计数
8. `query_impact` 能追踪跨 repo 调用链（repo A 代码引用 repo B 的 API 路径 → 跨 repo 边）
9. `last-scan.json` 按 repo 独立追踪，单 repo 变更只重扫该 repo
10. arch-engine + mcp-server 全测试绿（含新增 Go + Python scanner + workspace 测试）
...
- `lang`：`"java"` | `"go"` | `"python"` | `"ts"`（决定走哪个 scanner）
...
- `initWorkspace(projectRoot)` 检测文件清单：pom.xml（Java）/ go.mod（Go）/ pyproject.toml · setup.py · requirements.txt（Python）/ package.json（TS）/ *.csproj（预留）
...
**gRPC 服务（.proto 文件）：**
- 扫 `*.proto`，提取 `service PaymentService { rpc Charge(ChargeReq) returns (ChargeResp); }`（解析逻辑抽到共享 `proto-scanner.ts`，Python scanner 复用）
...
### 5.6b Python Scanner（python-scanner.ts）

缩进感知的正则提取（Python 是缩进式语法，函数体需按缩进块解析才能拿到方法体调用边）。与 Go scanner 同为正则路线，但需额外处理缩进层级。

**模块发现：**
- 扫 `pyproject.toml`（PEP 621 `[project] name`）或 `setup.py`（`setup(name=...)`）；均无则用目录名
- 依赖文件 `requirements.txt` / `Pipfile` / `pyproject.toml [project.dependencies]` 用于 stack 标注（不强制）

**HTTP API 提取（4 框架）：**
- FastAPI：`@app.get("/path")` / `@app.post(...)` / `@router.get(...)` / `APIRouter()`
- Flask：`@app.route("/path", methods=["GET"])` / `@bp.route(...)` / `@app.get(...)`
- Django：`urlpatterns = [path("foo/", views.foo), re_path(r"^api/", ...)]`
- Tornado：`Application([(r"/path", Handler)])` / `app.add_handlers`
- 提取 method + path + handler 函数名 → PythonApiEndpoint

**gRPC 服务（.proto 文件）：**
- 复用 `proto-scanner.ts`（与 Go scanner 共享同一 `.proto` 解析）
- 每个 rpc 方法 → PythonApiEndpoint（framework: "grpc"）

**ORM / 实体提取：**
- SQLAlchemy：`class X(Base): __tablename__ = "x"` + `Column(...)` / `mapped_column(...)`
- Pydantic：`class X(BaseModel):` + 字段 `name: type = ...`
- Django：`class X(models.Model):` + `field = models.CharField(...)`
- → PythonClass（`ormType` 标注来源，决定是否进 entity graph）

**函数定义 + 调用图：**
- `def func(...)` / `async def func(...)` → PythonMethodNode
- 类方法 `def method(self, ...)` → 标注 className
- 调用边（缩进块内）：`self.method()`（同类）/ `self.repo.method()`（已知字段）/ `await client.method()` / 裸调用 `foo()`
- receiver 分类沿用 Java/Go 模式：self → 同类方法 / 已知字段 → 字段类型方法 / 未知 → 丢弃
...
- Python repo → 新增 python-scanner（entity + flow + call-graph 三合一）
...
> 框架覆盖：topology 汇总含 Java（Spring）+ Go（gin/echo/chi/grpc）+ Python（FastAPI/Flask/Django/Tornado/grpc）+ 前端三类。
...
arch-engine:
1. types.ts（WorkspaceRepo/WorkspaceConfig/GoModule/GoStruct/GoApiEndpoint/GoMethodNode/PythonModule/PythonClass/PythonApiEndpoint/PythonMethodNode + JavaModule.repoSlug?）
2. workspace.ts（新增 — 清单加载 + init）
3. scanners/go-scanner.ts（新增 — Go 全量 scanner）
4. scanners/proto-scanner.ts（新增 — .proto 解析，Go/Python 共享）
5. scanners/python-scanner.ts（新增 — Python 全量 scanner）
6. scanners/registry.ts（Go + Python scanner 注册 + repo 路由）
7. pipeline.ts（workspace-aware 多根执行）
8. config.ts（workspace config 校验）
9. paths.ts（workspace-aware arch dir）
10. index.ts（导出新类型 + workspace loader + go/python scanner）
11. scanners/java.ts（JavaModule 加 repoSlug）
12. scanners/frontend.ts（FrontendPackage 加 repoSlug）

mcp-server:
13. ontology-query.ts（repoCount topology + repo 分组）
14. impact-query.ts（跨 repo 边标注）
15. ontology/types.ts（OntologyTopology repoCount）

测试:
16. tests/scanners/python-scanner.test.ts（FastAPI/Flask/SQLAlchemy/Pydantic/Django fixture + 调用图）

总计 16 文件，> 8 → high risk。
...
- **2.1.0 跨仓库 Workspace + Go + Python 语言 — 本 spec**
- 2.1.1 C# / Rust scanner — Planned
